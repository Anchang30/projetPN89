...
if "pn89_log_error" not in index_list :
    es.indices.create(index="pn89_log_error", mappings=mapping_barrier)

# Function to add errors as log in ES with class instanciation and error_type
def add_log_error(bar: Barrier, error_type: str):
    doc = bar.__dict__.copy()
    doc["error_type"] = error_type 
    es.index(index="log_error_barriers_pn89", document=doc)
